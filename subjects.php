...
<body>
	...
	<p id="time"> Time Remaining until lesson: </p>
	...
	<script type="text/javascript">
		...
		window.onload = function() {
			var time = prompt("How long would you like to take a study break, please enter in seconds?");
			var length = prompt("How many questions would you like to be asked? ");
...
countDownObj.style.fontFamily = 'Century Gothic';
...
elemEnglish.addEventListener("click", elemEnglishQuiz, false);
	...
	var hist = document.getElementById("hsHistory");
	hist.addEventListener("click", hsHistoryQuiz, false);
	*/
	var hsMath = {
	...
	var hsHistory = {
		"history": [ { "question": "Inventor and businessman. He developed many devices that greatly influenced life around the world, including the phonograph, the motion picture camera, and a long-lasting, practical electric light bulb.", "answer": "Thomas Edison"},
		{"question" : "This was the first permanent English settlement in the New World (1607)", "answer": "Jamestown"},
		{"question" : "What act prohibited all foreign trade in the ealry 1800s \nA. Secition Act \n B. Embargo Act \n C.No-Trade Act \n D. 1807 Act", "answer": "B"},
		{"question": "This treaty ended the War of 1812, what is the Treat of ", "answer": "Ghent"},
		{"question": "Who invented the cotton gin?", "answer": "Eli Whitney"},
		{"question": "Which was the first major battle of the American Revolution \n A. Battle of Bunker Hill \n B. Battle of Trenton \n C. World War II \n D. Battle of Yorktwon", "answer": "A"},
		{"question":"The three-fifths Compromise stated that 3/5ths of a state\'s ______ population would be counted for purposes of taxation and representation", "answer": "slave"},
		{"question": "The division of power between the state and national governments is called", "answer": "Federalism"},
		{"question": "Which country was U.S territory purchased from as a reuslt of the 1803 Lousiana Purchase?", "answer": "France"},
		{"question": "Group of Puritan separatists who established Plymouth Colony in Massachusetts to seek religious freedom after having lived briefly in the Netherlands.", "answer": "Pilgrims"},
		{"question": "Who was the English Quaker who founded Pennsylvania?", "answer": "William Penn"}, 
		{"question": "Which amendement dealt with prohibiting alcohol?", "answer": "18"},
		{"question": "An incident in which British soldiers fired into a crowd of colonists who were teasing and taunting them; five colonists were killed. This is known as The ", "answer": "Boston Massacre"},
		{"question": "Lexington and Concord was the site of the first shots of what Revolution?", "answer": "American"},
		{"question": "The Declaration of Independence was signed during what year? ", "answer": "1776"},
		{"question": "The first transcontinental expedition to the Pacific Coast by the United States was done by", "answer": "Lewis Clark"},
		{"question": "The Treaty of \n A. 1816 \n B. 1836 \n C. 1812 \n D. 1818 allowed for the Americans to shar Newfounded fisheries with Canada", "answer": "D"},
		{"question": "This party formed in opposition to the policies of Andrew Jackson and his Democratic Party. What is the ______ party", "answer": "Whig"},
		{"question": "Statement delivered by President James Monroe stating that further efforts by European nations to colonize land or interfere with states in North or South America would be viewed as acts of aggression requiring U.S. intervention is known as the", "answer": "Monroe Doctrine"},
		{"question": "Plessy V Ferguson is the famous test case remembered by the phrase Seperate but _____", "answer": "Equal"},
		{"question": "The court case Roe vs Wade dealt with ____", "answer": "Abortion"},
	...
	function hsMathQuiz()
	{
		for(var i = 0; i < num; i++)
		{
			...
			if(isEqual(input, hsMath.math[i].answer))
			{
				alert("CORRECT");
			}
			else
	...
	function hsScienceQuiz()
	{
		for(var i = 0; i < num;i++)
		{
			...
			if(isEqual(input , hsScience.science[i].answer))
			{
				alert("CORRECT");
			}
			else
	...
	function hsEnglishQuiz()
	{
		for(var i = 0; i < num; i++)
		{
			...
			if(isEqual(input,  hsEnglish.english[i].answer))
			{
				alert("Correct");
			}
			else
	...
	function hsHistoryQuiz()
	{
		for(var i = 0; i < num; i++)
		{
			var input = prompt(hsHistory.history[i].question);

			if(isEqual(input, hsHistory.history[i].answer))
			{
				alert("Correct");
			}
			else
			{
				alert("INCORRECT the correct answer was " + hsHistory.history[i].answer);
			}
	...
	function elemMathQuiz()
	{
		for(var i = 0; i < num; i++)
		{
			...
			if(isEqual(input,  elem_school.math[i].answer))
			{
				alert("Correct");
			}
			else
	...
	function elemScienceQuiz()
	{
		for(var i = 0; i < num; i++)
		{
			...
			if(isEqual(input, elem_school.science[i].answer))
			{
				alert("Correct");
			}
			else
	...
	function elemHistoryQuiz()
	{
		for(var i = 0; i < num; i++)
		{
			...
			if(isEqual(input, elem_school.history[i].answer))
			{
				alert("Correct");
			}
			else
	...
	function elemEnglishQuiz()
	{
		for(var i = 0; i < num; i++)
		{
			...
			if(isEqual(input, elem_school.english[i].answer))
			{
				alert("Correct");
			}
			else
	...
	function middleMathQuiz()
	{
		for(var i = 0; i < num;i++)
		{
			...
			if(isEqual(input , middle_school.math[i].answer))
			{
				alert("Correct");
			}
			else
	...
	function middleScienceQuiz()
	{
		for(var i = 0; i < num;i++)
		{
			...
			if(isEqual(input, middle_school.science[i].answer))
			{
				alert("Correct");
			}
			else
	...
	function middleEnglishQuiz()
	{
		for(var i = 0; i < num;i++)
		{
			...
			if(isEqual(input, middle_school.english[i].answer))
			{
				alert("Correct");
			}
			else
	...
	function middleHistoryQuiz()
	{
		for(var i = 0; i < num; i++)
		{
			...
			if(isEqual(input, middle_school.history[i].answer))
			{
				alert("Correct");
			}
			else
